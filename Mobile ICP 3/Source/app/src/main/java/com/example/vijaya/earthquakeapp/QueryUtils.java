...
public class QueryUtils {
    ...
    public static List<Earthquake> fetchEarthquakeData2(String requestUrl) {
        ...
        try {
            ...
            JSONArray finalResult = json.getJSONArray("features");
            for (int i = 0; i < finalResult.length(); i++) {
                JSONObject responseJson = finalResult.getJSONObject(i).getJSONObject("properties");
                double mag = responseJson.getDouble("mag");
                String place = responseJson.getString("place");
                long time = responseJson.getLong("time");
                String siteurl = responseJson.getString("url");
                Earthquake earthquake = new Earthquake(mag, place, time, siteurl);
